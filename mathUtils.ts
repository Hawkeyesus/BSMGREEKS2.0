 ...
 * (High precision Abramowitz and Stegun formula)
 */
...
export function calculateBSM(inputs: BSMInputs): BSMOutputs {
  ...
  // Time to expiry calculation (years)
  const today = new Date();
  const expiry = new Date(expiryDate);
  const diffTime = expiry.getTime() - today.getTime();
  const T = Math.max(diffTime / (1000 * 60 * 60 * 24 * 365.25), 0.0001); // Avoid T=0
  ...
  if (optionType === 'call') {
    ...
    theta = (
      -(S * Math.exp(-q * T) * v * normPdf(d1)) / (2 * Math.sqrt(T)) -
...
export function generateSurfaceData(
  ...
  // Grid of 30x30 for performance and smoothness
  const spotRange = Array.from({ length: 30 }, (_, i) => inputs.spot * (0.6 + (i * 0.8) / 30));
  const timeRange = Array.from({ length: 30 }, (_, i) => (i + 1) / 30); // 0 to 1 year

  const z: number[][] = [];

  for (let tIdx = 0; tIdx < timeRange.length; tIdx++) {
    const row: number[] = [];
    const T = timeRange[tIdx];
    const expirySim = new Date(Date.now() + T * 365.25 * 24 * 60 * 60 * 1000).toISOString();
    
    for (let sIdx = 0; sIdx < spotRange.length; sIdx++) {
      const S = spotRange[sIdx];
      const result = calculateBSM({
        ...inputs,
        spot: S,
        expiryDate: expirySim
      });
...
export function generateHeatmapData(
  ...
  const steps = 15;
  const volRange = Array.from({ length: steps }, (_, i) => inputs.volatility * (0.5 + (i / steps) * 1.5));
  const spotRange = Array.from({ length: steps }, (_, i) => inputs.spot * (0.8 + (i / steps) * 0.4));

  const z: number[][] = [];
  const xLabels = spotRange.map(s => `$${s.toFixed(2)}`);
