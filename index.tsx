...
import { calculateBSM as calculateBSMAPI, formatApiError, ApiError } from './api';
import './index.css';
...
const App = () => {
  ...
  const syncBackend = async () => {
    ...
    setError(null);
    try {
      const data = await calculateBSMAPI(inputs);
      setBackendData(data);
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#09090b] text-zinc-100">
      ...
      <aside className="w-full lg:w-80 border-r border-zinc-800 bg-zinc-900/20 p-6 flex flex-col gap-6 overflow-y-auto shrink-0">
        ...
        <div className="mt-6 pt-6 border-t border-zinc-800 space-y-4">
           ...
           <div className="flex flex-col gap-1">
             <div className={`text-[10px] font-bold transition-colors ${isSyncing ? 'text-emerald-500 animate-pulse' : error ? 'text-red-500' : 'text-zinc-600'}`}>
               {isSyncing ? 'SYNCING ENGINE...' : error ? 'ENGINE ERROR' : 'ENGINE READY'}
             </div>
